...
const withDiscordInteraction = (next: any) => async (
    ...
    try {
        ...
        if (customBot.owner.role !== "business" && customBot.owner.role !== "enterprise") return res.status(200).json({ type: 4, data: { content: "This feature is only available with the Business and Enterprise Subscriptions. Please [upgrade](https://restorewhore.com/dashboard/upgrade) your subscription to utilize this feature.", flags: 64 } });
