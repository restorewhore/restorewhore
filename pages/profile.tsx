...
export default function Verify({ info, servers, err }: { info: UserInfo, servers: Server[], err: string }) {
    const hostname = info.domain ? (info.domain.match(/(?:[^.]+\.)?([^.\s]+)\.[^.]+$/) || [])[1] || info.domain : "restorewhore.com";
    const domain = info.domain ? (info.domain.match(/(?:[^.]+\.)?([^.]+\.[^.]+)$/) || [])[1] || info.domain : "restorewhore.com";
    ...
    return (
        <>
            <Head>
                ...
                <meta property="og:url" content={`https://${info.domain ?? "restorewhore.com"}`} />
...
export async function getServerSideProps({ req }: any) {
    if (req) {
        const domain = req.headers.host ?? "restorewhore.com";
        ...
        servers = userServers.filter((server) => server.locked === false).map((server) => {
            return {
                ...
                picture: server.picture ?? "https://restorewhore.com/logo512.png",
            }
        });
